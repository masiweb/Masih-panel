...
inbounds=async function(){const [r,n]=await Promise.all([req("/admin/inbounds"),req("/nodes")]);inboundsData=r;inboundNodes=n;view.innerHTML=`<div class="panel"><div class="panel-heading"><div><h2>Inboundها</h2><p>ساخت و ویرایش کامل شنونده‌های VPN با فرم‌های استاندارد هر پروتکل</p></div><button class="primary" data-action="openInboundModal">＋ افزودن ورودی</button></div><div class="protocol-summary"><span>VLESS · VMess · Trojan · Shadowsocks</span><span>RAW · mKCP · WebSocket · gRPC · HTTPUpgrade · XHTTP</span><span>TLS · Reality</span></div><div class="table-wrap"><table><thead><tr><th>فعال</th><th>نام</th><th>پروتکل</th><th>انتقال</th><th>امنیت</th><th>Node</th><th>Listen / Port</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>${r.map(x=>`<tr><td><span class="status ${x.enabled?"active":"revoked"}">${x.enabled?"فعال":"خاموش"}</span></td><td><b>${esc(x.name)}</b><small class="row-note">${esc(x.remark||"")}</small></td><td><span class="tag">${esc(x.settings?.variant||x.protocol)}</span></td><td>${esc(x.settings?.transport||"-")}</td><td>${esc(x.settings?.security||"-")}</td><td>${esc(n.find(v=>v.id===x.node_id)?.name||x.node_id)}</td><td dir="ltr">${esc(x.listen)}:${esc(x.port)}</td><td><span class="status ${esc(x.status)}">${esc(x.status)}</span></td><td class="row-actions"><button data-action="editInbound" data-id="${x.id}">ویرایش</button><button class="danger" data-action="deleteInbound" data-id="${x.id}">حذف</button></td></tr>`).join("")}</tbody></table></div></div>`;bindActions()};
